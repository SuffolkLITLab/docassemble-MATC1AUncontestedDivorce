...
def load_scenarios(filter_name=None):
    ...
    for path in sorted(SCENARIOS_DIR.glob("*.json")):
        if path.name.startswith("."):
            continue
        # Skip _-prefixed files UNLESS the filter explicitly matches them
        if path.name.startswith("_"):
            if not (filter_name and filter_name in path.stem):
                continue
        try:
...
def _is_class_name(var_name):
    """Check if a variable name looks like a Python/docassemble class name.

    Class names are PascalCase identifiers without dots or brackets.
    Setting these to True in the interview namespace corrupts the runtime
    (e.g., DAAddress=True makes DAAddress() uncallable).
    ...
    Examples:
        ...
        MACourt -> True
        Individual -> True
        user_employer_name -> False
        users[0].name -> False
        income_list.selected_types -> False
    """
    ...
    if not var_name[0].isupper():
        return False
    # Must not contain dots, brackets, or spaces (those are attribute paths)
        ...
        for step in range(1, max_steps + 1):
            ...
            if q_type == "undefined_variable":
                ...
                if undef_var:
                    ...
                    if undef_var in test_data:
                        value = test_data[undef_var]
                        if verbose or debug:
                            details.append(
            ...
            if q_type == "settrue":
                ...
                for field in extra_fields:
                    ...
                    if resolved in test_data:
                        variables[send_var] = test_data[resolved]
                    elif vn != resolved and vn in test_data:
                        variables[send_var] = test_data[vn]
                    else:
                ...
                try:
                    ...
                    while resp_type == "undefined_variable" and undef_resolve_count < 5:
                        ...
                        if not undef_var:
                            break
                        # SAFETY: Never set class names (PascalCase) to True.
                        # Docassemble may report class names like DAAddress,
                        # ALIncomeList, etc. as "undefined" during object
                        # creation. Setting these to True corrupts the
                        # namespace and causes 'bool not callable' errors.
                        if _is_class_name(undef_var):
                            if debug:
                                print(f"    {CYAN}  SKIP class name '{undef_var}' (not setting){RESET}")
                            details.append(
                                f"Step {step}: Skipping class name '{undef_var}' "
                                f"(post-settrue, will let DA resolve)"
                            )
                            break
                        attempt_count = undefined_var_attempts.get(undef_var, 0)
                        if attempt_count >= 2:
            ...
            for field in fields:
                ...
                if resolved_var in test_data:
                    value = test_data[resolved_var]
                elif raw_var != resolved_var and raw_var in test_data:
                    value = test_data[raw_var]
                else:
            ...
            if not variables and q_id:
                ...
                if snake_id in test_data:
                    variables[snake_id] = test_data[snake_id]
                else:
            ...
            try:
                ...
                while resp_type == "undefined_variable" and undef_resolve_count < 5:
                    ...
                    if not undef_var:
                        break
                    # SAFETY: Never set class names (PascalCase) to True.
                    # See _is_class_name() docstring for details.
                    if _is_class_name(undef_var):
                        if debug:
                            print(f"    {CYAN}  SKIP class name '{undef_var}' (not setting){RESET}")
                        details.append(
                            f"Step {step}: Skipping class name '{undef_var}' "
                            f"(post-fields, will let DA resolve)"
                        )
                        break
                    attempt_count = undefined_var_attempts.get(undef_var, 0)
                    if attempt_count >= 2:
                        break
                    undefined_var_attempts[undef_var] = attempt_count + 1
                    val = test_data.get(undef_var, True)
                    if verbose or debug:
                        details.append(
                            f"Step {step}: Setting undefined '{undef_var}' "
                ...
                # Diagnostic: try to get current question without setting vars
                if debug and "TypeError" in resp_text:
                    details.append(f"  --- DIAGNOSTIC: probing session state ---")
                    try:
                        diag_resp = client.get_question(session_id, secret, i)
                        details.append(f"  GET question OK: id={diag_resp.get('id','?')}")
                    except Exception as diag_e:
                        diag_get_text = ""
                        if hasattr(diag_e, "response") and diag_e.response:
                            try:
                                diag_get_text = diag_e.response.text[:500]
                            except Exception:
                                pass
                        details.append(f"  GET question FAILED: {diag_get_text or diag_e}")
                    # Try setting just one variable at a time
                    for k, v in variables.items():
                        try:
                            client.set_variables(session_id, secret, i, {k: v})
                            details.append(f"  SET '{k}'={repr(v)[:60]} → OK")
                        except Exception as diag_e2:
                            diag_text = ""
                            if hasattr(diag_e2, "response") and diag_e2.response:
                                try:
                                    diag_text = diag_e2.response.text[:500]
                                except Exception:
                                    pass
                            details.append(
                                f"  SET '{k}'={repr(v)[:60]} → FAIL: {diag_text or diag_e2}"
                            )
                    # Also probe: after has_attorney=False succeeded, try GET
                    # to see what the current question is now
                    try:
                        diag_resp2 = client.get_question(session_id, secret, i)
                        details.append(
                            f"  After has_attorney SET, GET: id={diag_resp2.get('id','?')}, "
                            f"type={diag_resp2.get('questionType','?')}"
                        )
                    except Exception as diag_e3:
                        diag_text3 = ""
                        if hasattr(diag_e3, "response") and diag_e3.response:
                            try:
                                diag_text3 = diag_e3.response.text[:500]
                            except Exception:
                                pass
                        details.append(f"  After has_attorney SET, GET FAILED: {diag_text3 or diag_e3}")
...
def main():
    ...
    # Load scenarios ("all" means no filter)
    filter_name = args.scenario if args.scenario and args.scenario.lower() != "all" else None
