...
class DocassembleClient:
    ...
    def get_variables(self, session_id, secret, i):
        """Get the current interview variable dictionary."""
        params = {
            "key": self.api_key,
            "i": i,
            "session": session_id,
    ...
    def download_file(self, file_number, output_path):
        """Download a generated docassemble file by file number."""
        r = self.session.get(
            f"{self.server}/api/file/{file_number}",
            params={"key": self.api_key, "extension": "pdf"},
            timeout=60,
        )
        r.raise_for_status()
...
def normalize_variable_name(name):
    """Normalize docassemble generic-list names for test data lookup."""
    if not name:
        return name
    return re.sub(r"\[(\d+)\]\[\1\]", r"[\1]", name)
...
def resolve_generic_variable(field_var, question_var_name):
    ...
    # Replace literal [i] with the index from question_variable_name for
    # package-authored generic list fields like schedule_a_expenses[i].source.
    qvn_index_match = re.search(r'\[(\d+)\]', question_var_name)
    if "[i]" in field_var and qvn_index_match:
        field_object = field_var.split("[", 1)[0]
        question_object = question_var_name.split("[", 1)[0]
        if field_object == question_object:
            return field_var.replace("[i]", "[" + qvn_index_match.group(1) + "]", 1)
    ...
    obj_match = re.match(r"^([A-Za-z_]\w*)", question_var_name)
    obj_name = obj_match.group(1) if obj_match else question_var_name
...
def safe_path_name(text):
    """Return a filesystem-safe name for test output directories."""
    return re.sub(r"[^A-Za-z0-9_.-]+", "_", str(text)).strip("_") or "scenario"


def collect_generated_pdfs(value, found=None):
    """Recursively collect generated PDF DAFile objects from session variables."""
    if found is None:
        found = {}
    if isinstance(value, dict):
        if (
            value.get("_class") == "docassemble.base.util.DAFile"
            and value.get("extension") == "pdf"
            and value.get("number") is not None
            and value.get("filename")
        ):
            found[value["filename"]] = {
                "number": value["number"],
                "title": value.get("title", ""),
            }
        for child in value.values():
            collect_generated_pdfs(child, found)
...
def pdf_field_values(path):
    """Read AcroForm field values from a generated PDF."""
    ...
    for field_name, field in fields.items():
        ...
        if raw_value is None:
            ...
            values[field_name] = str(raw_value).lstrip("/")
    return values


def check_expected_value(actual, expected):
    """Return (ok, message) for a PDF field expectation."""
    actual_text = "" if actual is None else str(actual)
    if isinstance(expected, dict):
        if expected.get("not_blank"):
            if actual_text.strip():
                return True, ""
            return False, "expected a nonblank value"
        if "equals" in expected:
            expected_text = str(expected["equals"])
            if actual_text == expected_text:
                return True, ""
            return False, f"expected {expected_text!r}, got {actual_text!r}"
        if "contains" in expected:
            expected_text = str(expected["contains"])
            if expected_text in actual_text:
                return True, ""
            return False, f"expected value containing {expected_text!r}, got {actual_text!r}"
        if "one_of" in expected:
            allowed = [str(item) for item in expected["one_of"]]
            if actual_text in allowed:
                return True, ""
            return False, f"expected one of {allowed!r}, got {actual_text!r}"
        return False, f"unsupported expectation object {expected!r}"
...
def assert_pdf_outputs(client, session_id, secret, i, scenario, output_dir, details):
    """Download generated PDFs and assert scenario-declared AcroForm values."""
    expected = scenario.get("expected_pdf_fields", {})
    if not expected:
        return True, "No PDF assertions declared"

    variables = client.get_variables(session_id, secret, i)
    generated_pdfs = collect_generated_pdfs(variables)
    scenario_slug = safe_path_name(Path(scenario.get("_file", scenario.get("name", "scenario"))).stem)
    scenario_output = Path(output_dir) / scenario_slug
    scenario_output.mkdir(parents=True, exist_ok=True)
    (scenario_output / "session_variables.json").write_text(
        json.dumps(variables, indent=2, sort_keys=True, default=str)
    )
    (scenario_output / "generated_pdfs.json").write_text(
        json.dumps(generated_pdfs, indent=2, sort_keys=True, default=str)
    )

    failures = []
    for pdf_name, field_expectations in expected.items():
        actual_pdf_name = pdf_name if pdf_name.endswith(".pdf") else f"{pdf_name}.pdf"
        if actual_pdf_name not in generated_pdfs:
            failures.append(
                f"{actual_pdf_name}: not generated; available PDFs: {sorted(generated_pdfs)}"
            )
            continue

        file_number = generated_pdfs[actual_pdf_name]["number"]
        pdf_path = scenario_output / actual_pdf_name
        try:
            client.download_file(file_number, pdf_path)
        except Exception as err:
            failures.append(f"{actual_pdf_name}: download failed: {err}")
            continue

        try:
            values = pdf_field_values(pdf_path)
        except Exception as err:
            failures.append(f"{actual_pdf_name}: could not read PDF fields: {err}")
            continue

        details.append(f"Downloaded {actual_pdf_name} to {pdf_path}")
        for field_name, expected_value in field_expectations.items():
            if field_name not in values:
                failures.append(f"{actual_pdf_name}: field {field_name!r} not found")
                continue
            ok, message = check_expected_value(values[field_name], expected_value)
            if not ok:
                failures.append(f"{actual_pdf_name}: {field_name}: {message}")

    if failures:
        details.extend(f"PDF assertion failed: {failure}" for failure in failures)
        return False, f"{len(failures)} PDF assertion failure(s)"
    return True, "PDF assertions passed"


def run_scenario(client, scenario, verbose=False, debug=False, output_dir=None):
    ...
    def lookup_test_value(*names):
        ...
        for name in names:
            if not name:
                continue
            for candidate in dict.fromkeys([name, normalize_variable_name(name)]):
                seq_key = f"{candidate}__sequence"
                ...
                if candidate in test_data:
                    return True, test_data[candidate]
        return False, None
    ...
    try:
        for step in range(1, max_steps + 1):
            ...
            screen_signature = (
                q_id,
                qvn,
                tuple(question.get("event_list", []) or []),
            )
            if q_id and screen_signature == prev_screen_signature:
                stuck_count += 1
                if stuck_count >= MAX_STUCK:
                    details.append(
                        f"Step {step}: STUCK on screen '{q_id}' seeking "
                        f"'{qvn}' for {stuck_count} consecutive steps"
                    )
                    if debug:
                        dump_question_debug(question, step)
                ...
                stuck_count = 0
            prev_screen_signature = screen_signature
            ...
            if is_terminal_screen(question):
                ...
                if scenario.get("expected_pdf_fields"):
                    ok, pdf_message = assert_pdf_outputs(
                        client,
                        session_id,
                        secret,
                        i,
                        scenario,
                        output_dir or (SCRIPT_DIR / "test_output"),
                        details,
                    )
                    if not ok:
                        return False, step, pdf_message, details
                    details.append(pdf_message)
            ...
            for field in fields:
                ...
                if not visible and send_var in variables and resolved_var != qvn:
